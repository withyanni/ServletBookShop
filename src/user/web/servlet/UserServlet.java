...
public class UserServlet extends BaseServlet
{
	...
	//该层依赖UserService，所以创建一个UserService
	...
	{
		String string=request.getParameter("loginname");
		boolean b=userService.ajaxValidateLoginname(string);
	...
	public String updatePassword(HttpServletRequest request ,
			HttpServletResponse respons) throws SQLException,IOException
	{
		/*
		 * 封装表单数据到user中
		 * 从session中获取uid
		 * 使用uid和表单中的oldPass和newPass来调用service方法
		 * 	如果出现异常，保存异常信息到request中，转发到pws.jsp
		 * 保存成功信息到request中
		 * 转发到msg.jsp
		 */
			...
			throws ServletException,IOException,SQLException
	{
		 ...
		 * 4.查看用户是否存在
		 * 		如果不存在，保存错误信息，用户名或者密码错误，保存用户数据，为了回显，转发到login.jsp
		 * 5.如果存在，查看激活状态，如果状态为false
		 * 		保存错误信息，提示没有激活。保存表单数据，为了回显，转发到login.jsp
		 * 6.登录成功，
		 * 		保存当前查询到的user到session中，保存当前用户的名称到cookie，注意中文需要编码处理（cookie里不能显示中文）
		 * 
		 */
	...
	public String ajaxValidateEmail(HttpServletRequest request ,
			HttpServletResponse response) throws ServletException,IOException
	{
		String string=request.getParameter("email");
		boolean b=userService.ajaxValidateEmail(string);
		response.getWriter().print(b);
		return null;
	}
			...
			HttpServletResponse response) throws ServletException,IOException
	{
		 ...
		 * 	 service方法有可能抛出异常，把异常信息拿来，保存到request，转发到msg.jsp显示
